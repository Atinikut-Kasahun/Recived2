...
function Toast({ msg, type }: { msg: string; type: 'success' | 'error' }) {
    return (
        <motion.div
            ...
            className="fixed bottom-8 right-8 z-[200] px-6 py-4 rounded-2xl shadow-2xl text-[13px] font-black uppercase tracking-widest text-[#FDF22F] flex items-center gap-3 border border-[#FDF22F]/20 bg-black"
        >
...
function StatCard({ label, value, icon: Icon, trend, trendUp, accent }: any) {
    return (
        <motion.div
            ...
            className={`relative bg-white rounded-[28px] border border-gray-100 shadow-sm overflow-hidden p-7 group cursor-default`}
        >
            {accent && (
                <div className="absolute top-0 left-0 right-0 h-1 bg-[#FDF22F]" />
            )}
            <div className="flex items-start justify-between mb-5">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${accent ? 'bg-[#FDF22F]' : 'bg-gray-50 border border-gray-100'}`}>
                    <Icon size={20} className={accent ? 'text-black' : 'text-gray-400'} />
                </div>
                <span className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest flex items-center gap-1 ${trendUp ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
    ...
    const fetchData = async () => {
        try {
            ...
            if (localTab === 'JOBS') {
                ...
                const status = subTab === 'PENDING' ? 'pending' : 'all';
        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="flex justify-between items-end mb-4">
                <div className="space-y-4">
                    <div className="flex items-center border-b border-gray-100">
                        ...
                        <div className="flex gap-8 ml-10 border-l border-gray-100 pl-10 mb-3 h-5 items-center">
                            {(() => {
                                let tabs: string[] = [];
                ...
                {localTab === 'HIRING PLAN' && selectedIds.length > 0 && (
                    <button onClick={handleBulkApprove} disabled={actionLoading} className="bg-[#FDF22F] hover:bg-black text-[#000000] hover:text-[#FDF22F] px-8 py-3.5 rounded-2xl font-black text-[13px] tracking-widest uppercase shadow-xl shadow-[#FDF22F]/10 transition-all flex items-center gap-2 group border border-[#FDF22F]">
            ...
            {loading ? (
                ...
                <div className="bg-white rounded border border-gray-100 shadow-sm overflow-hidden">
                            ...
                            {jobsMeta?.last_page > 1 && (
                                <div className="px-8 py-6 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button disabled={jobsPage === 1} onClick={() => setJobsPage(p => p - 1)} className="w-10 h-10 rounded-xl border border-gray-200 bg-white flex items-center justify-center text-gray-400 hover:border-[#FDF22F] hover:text-black transition-all disabled:opacity-30">
                                            <ChevronLeft size={18} />
                                        </button>
                                        ...
                                        <button disabled={jobsPage === jobsMeta.last_page} onClick={() => setJobsPage(p => p + 1)} className="w-10 h-10 rounded-xl border border-gray-200 bg-white flex items-center justify-center text-gray-400 hover:border-[#FDF22F] hover:text-black transition-all disabled:opacity-30">
                    ...
                    {/* ══════════════════════════════════════════════════════
                        REPORTS TAB — Redesigned Professional Dashboard
                    ══════════════════════════════════════════════════════ */}
                    {localTab === 'REPORTS' && (
                        <div className="bg-[#F5F6FA] min-h-screen flex">

                            {/* ── Left Sidebar ── */}
                            <div className="w-60 bg-white border-r border-gray-100 flex flex-col shrink-0" style={{ minHeight: '800px' }}>
                                {/* Brand */}
                            ...
                            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                                ...
                                {/* ── Top Bar ── */}
                                <div className="bg-white border-b border-gray-100 px-8 py-4 flex justify-between items-center shadow-sm">
                                    ...
                                    <div className="flex items-center gap-5">
                                        ...
                                        <select
                                            ...
                                            className="text-[11px] font-black uppercase tracking-widest bg-white border border-gray-200 rounded-xl px-4 py-2.5 outline-none focus:border-[#FDF22F] cursor-pointer"
                                        >
                                        ...
                                        <button className="relative w-10 h-10 flex items-center justify-center text-gray-400 hover:text-black transition-colors rounded-xl hover:bg-gray-50 border border-gray-100">
                                ...
                                <div className="p-8 space-y-8 overflow-y-auto flex-1">
                                    ...
                                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-center">
                                        <div>
                                            <h2 className="text-[32px] font-black text-black tracking-tight leading-none">
                                            ...
                                            <p className="text-gray-400 text-[13px] font-medium mt-2 uppercase tracking-[0.2em]">Talent Acquisition · Real-time Overview</p>
                                        </div>
                                        <div className="flex gap-3">
                                            <button className="flex items-center gap-2 bg-white border border-gray-200 px-6 py-3 rounded-2xl font-black text-[11px] tracking-widest uppercase hover:border-black transition-all shadow-sm">
                                                <Download size={14} /> Export Report
                                            </button>
                                            <button className="flex items-center gap-2 bg-black text-[#FDF22F] px-7 py-3 rounded-2xl font-black text-[11px] tracking-widest uppercase shadow-xl shadow-black/10 hover:bg-[#FDF22F] hover:text-black transition-all border border-black">
                                    ...
                                    <div className="grid grid-cols-3 gap-6">
                                        ...
                                        <div className="col-span-2 bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
                                            <div className="flex justify-between items-start mb-6">
                                                <div>
                                                    <h3 className="text-[18px] font-black text-black tracking-tight">Employee Turnover</h3>
                                            ...
                                            <div className="flex justify-between mt-3 px-1">
                                                {['Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Jan', 'Feb', 'Mar'].map(m => (
                                                    <span key={m} className="text-[10px] font-black text-gray-300 uppercase tracking-tighter">{m}</span>
                                                ))}
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
                                            <div className="mb-6">
                                                <h3 className="text-[18px] font-black text-black tracking-tight">Hiring Funnel</h3>
                                    ...
                                    <div className="grid grid-cols-3 gap-6">
                                        ...
                                        <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
                                            <div className="flex items-center justify-between mb-6">
                                                <h3 className="text-[16px] font-black text-black">By Department</h3>
                                        ...
                                        {/* Time to Hire */}
                                        <div className="bg-black rounded-[32px] shadow-sm p-8 relative overflow-hidden">
                                            ...
                                            <div className="relative z-10">
                                                <div className="flex items-center justify-between mb-6">
                                                    <h3 className="text-[16px] font-black text-white">Time to Hire</h3>
                                                    <Clock size={16} className="text-[#FDF22F]" />
                                                </div>
                                                <div className="text-center py-4">
                                                    <p className="text-7xl font-black text-[#FDF22F] tabular-nums leading-none">{stats?.avg_time_to_hire ?? '—'}</p>
                                        ...
                                        <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-8">
                                            <div className="flex items-center justify-between mb-6">
                                                <h3 className="text-[16px] font-black text-black">Live Activity</h3>
                                            ...
                                            <div className="space-y-4">
                                                {(recentApplicants.slice(0, 4).length > 0 ? recentApplicants.slice(0, 4) : Array(4).fill(null)).map((app, i) => (
                                                    <div key={i} className="flex items-center gap-3">
                                                        ...
                                                        <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg border ${statusColor[app?.status] ?? 'bg-gray-50 text-gray-400 border-gray-100'}`}>
                                    ...
                                    <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
                                        <div className="flex justify-between items-center px-10 py-7 border-b border-gray-50">
                                            <div>
                                                <h3 className="text-[18px] font-black text-black tracking-tight">Recent Applications</h3>
                                                <p className="text-[11px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1">Latest candidate submissions</p>
                                            </div>
                                            <div className="flex gap-3">
                                                <button className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 rounded-xl text-[10px] font-black uppercase tracking-widest hover:border-black transition-all">
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-8 py-5">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div>
                                                                    <p className="text-[13px] font-black text-black group-hover:text-[#000000] transition-colors">{app?.name ?? '—'}</p>
                                        ...
                                        <div className="px-10 py-5 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                                            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Showing {recentApplicants.length} recent entries</p>
                                            <button onClick={() => router.push('/dashboard?tab=Candidates')} className="text-[11px] font-black text-black uppercase tracking-widest hover:text-[#000000] flex items-center gap-1.5 bg-[#FDF22F] px-5 py-2.5 rounded-xl hover:bg-black hover:text-[#FDF22F] transition-all">
            ...
            <AnimatePresence>
                {drawerReq && (
                    <>
                        ...
                        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed right-0 top-0 bottom-0 w-full max-w-xl bg-white shadow-2xl z-[120] overflow-y-auto">
                                        ...
                                        {feedbackTarget === drawerReq.id ? (
                                            <div className="space-y-4">
                                                ...
                                                <div className="flex gap-3">
                                                    ...
                                                    <button onClick={handleAction} disabled={!feedbackReason.trim() || actionLoading} className={`flex-[2] px-6 py-4 text-white rounded-2xl text-[11px] font-black tracking-widest uppercase transition-all disabled:opacity-50 ${actionType === 'amend' ? 'bg-amber-600 hover:bg-amber-700' : 'bg-black hover:bg-red-600'}`}>Confirm & Notify GM</button>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="flex gap-3">
