...
export default function JobBoard({
  ...
  React.useEffect(() => {
    if (settings?.site_job_departments) {
      const dynamicDepts =
        typeof settings.site_job_departments === "string"
    ...
    const fetchJobs = async () => {
      ...
      try {
        ...
        if (activeCategory !== "All Departments")
          params.set("department", activeCategory);
        if (searchQuery) params.set("search", searchQuery);

        const response = await fetch(
          `${API_URL}/v1/public/jobs?${params.toString()}`,
        );
  ...
  React.useEffect(() => {
    if (searchQuery && jobs.length === 0 && !loading) {
      // Check !loading to ensure fetch is complete
    ...
    <section className="py-20 md:py-24 bg-white" id="jobs">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-10 md:mb-12 gap-8">
          ...
          >
            <span className="text-[#000000] font-bold text-[10px] md:text-xs uppercase tracking-widest mb-4 block">
              Available Roles
            </span>
        ...
        {showNoResultsMessage && (
          ...
          >
            <div className="w-12 h-12 md:w-16 md:h-16 bg-[#000000]/5 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
              <svg
                className="w-6 h-6 md:w-8 md:h-8 text-[#000000]/40"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <h3 className="text-[#000000] font-black text-xl md:text-2xl mb-2 md:mb-4">
              No matching opportunities found
            </h3>
            <p className="text-[#000000]/70 text-sm md:text-base max-w-lg mx-auto leading-relaxed">
              While we don&apos;t have an exact match for your current search
              criteria, we&apos;ve refreshed the view to show all open positions at{" "}
              <span className="font-bold text-[#000000]">Droga Group</span>.
            </p>
            <p className="text-[#000000]/50 text-[10px] md:text-sm mt-4 md:mt-6 font-medium italic">
              We&apos;ve automatically cleared the search filters for you.
            </p>
          </motion.div>
        )}
              ...
              >
                <div>
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="text-[#000000]/30 text-[9px] font-bold">
                      {job.type}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {(job.published_at || job.created_at) && (
                      <div className="bg-white/60 px-2 py-1 rounded-lg flex items-center gap-1 border border-[#000000]/5 shadow-sm">
                        <svg
                          className="w-3 h-3 text-[#000000]/40"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                ...
                <Link
                  ...
                  className="mt-4 w-full py-3 rounded-xl bg-transparent border border-black/10 text-black font-bold group-hover:bg-[#FDF22F] group-hover:text-black group-hover:border-[#FDF22F] transition-all text-center block"
                >
                  Apply Now <span>→</span>
                </Link>
              </motion.div>
